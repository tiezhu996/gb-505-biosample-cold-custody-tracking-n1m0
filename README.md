...
4. 冻存柜报警后，保管员登记温度异常处置单（起始温度、原因、处置人），未结案容器下样本自动纳入在控清单并暂缓交接与协议放行；选择转柜时逐支指定目标格位，容量、温区或格位不合适整单不生效；确认恢复后补记结束温度与结论，异常时段永久保留在样本页和处置记录中。
5. 所有关键写操作记录请求 ID、操作者、前后状态、前后位置和保管人，并使用 SHA-256 前向哈希形成只追加审计链。

首次启动会幂等创建 3 个冻存容器、4 份样本、2 条交接记录、1 条协议复核记录和 1 条已结案温度异常处置记录，便于直接验证完整流程。
...
| `GET /api/temperature-incidents[/:id]` | 查询温度异常处置单 | 已登录 |
| `POST /api/temperature-incidents` | 登记报警（起始温度、原因、处置人） | `incident:manage` |
| `POST /api/temperature-incidents/:id/relocate` | 逐支指定目标格位转柜 | `incident:manage` |
| `POST /api/temperature-incidents/:id/resolve` | 补结束温度与结论并结案 | `incident:manage` |
...
- 保管员对冻存柜登记处置单时，起始温度必须已超出该容器温区，并填写报警原因；容器自动置为 `alarm`，同一容器只允许一张未结案处置单。
- 登记瞬间在柜且未出库/未处置的样本全部快照进在控清单；未结案前，这些样本的交接发起、交接受理（含目标容器也在报警的情形）和协议“通过”放行都会在事务内被拦截。
- 转柜为可选项：需要在处置单中逐支指定目标容器与格位。目标容器必须可用且无未结案处置单、温区不得更暖、容量足够、格位未被占用（含单内重复）；任一支校验失败，整单不生效，不移动任何样本。留空的样本继续留柜观察。
- 确认恢复时补记结束温度、处置结论和恢复人，处置单结案、容器恢复为可用（人工维护中的容器保持维护状态），交接与放行解除暂缓。
- 样本页、样本详情抽屉永久保留异常时段（起止时间、起止温度、报警原因、结论、是否转柜及目标格位）；每次登记、转柜、结案均写入只追加审计链。
