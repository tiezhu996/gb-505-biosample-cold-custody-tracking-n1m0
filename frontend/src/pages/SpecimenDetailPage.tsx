...
import { ExceptionTimeline, SpecimenHoldTag, toExceptionEntries } from '../components/common/ExceptionTimeline'
...
export function SpecimenDetailPage() {
  ...
  const exceptionEntries = toExceptionEntries(specimen.temperatureItems)
  return (
    <div className="page-stack">
      <header className="page-header"><div><Button type="text" icon={<ArrowLeftOutlined />} onClick={() => navigate('/specimens')}>返回样本队列</Button><Typography.Title level={2}>{specimen.accessionNo}</Typography.Title></div><Space><SpecimenHoldTag specimen={specimen} /><CustodyBadge state={specimen.state} /></Space></header>
      ...
      <section className="detail-section"><Typography.Title level={4}>温度异常时段</Typography.Title><ExceptionTimeline entries={exceptionEntries} /></section>
