...
import { SpecimenHoldTag, specimenBlocked } from '../components/common/ExceptionTimeline'
...
export function TransfersPage() {
  ...
  const [createSpecimenId, setCreateSpecimenId] = useState<number>()
  ...
  const blockedSpecimenCount = specimens.filter((item) => specimenBlocked(item)).length
  const create = async () => {
    const values = await createForm.validateFields()
    setSaving(true)
    try { await transferAPI.create({ ...values, reason: values.reason || '' }); message.success('交接任务已发起'); setCreateOpen(false); createForm.resetFields(); setCreateSpecimenId(undefined); await refresh() } finally { setSaving(false) }
  }
  const resolve = async () => {
  ...
  const columns: ColumnsType<CustodyTransfer> = [
    ...
    { title: '样本', render: (_, row) => <div><strong>{row.specimen?.accessionNo || row.specimenId}</strong><small className="cell-subtitle">{row.specimen?.sampleType}</small>{specimenBlocked(row.specimen) && <div style={{ marginTop: 4 }}><SpecimenHoldTag specimen={row.specimen} /></div>}</div> },
    ...
    { title: '操作', fixed: 'right', render: (_, row) => <Space><Button size="small" icon={<HistoryOutlined />} onClick={() => void showTimeline(row)}>链路</Button>{row.state === 'prepared' && can('transfer:resolve') && <Button size="small" type="primary" icon={<CheckOutlined />} onClick={() => { setResolveTarget(row); setResolution(specimenBlocked(row.specimen) ? 'rejected' : 'accepted') }}>处理</Button>}</Space> },
  ]
  return (
    <div className="page-stack">
      ...
      {blockedSpecimenCount > 0 && <Alert type="error" showIcon message={`有 ${blockedSpecimenCount} 支样本处于未结案温度异常期间，已暂缓交接`} />}
      ...
      <Modal title="发起样本交接" width={700} open={createOpen} confirmLoading={saving} onOk={() => void create()} onCancel={() => setCreateOpen(false)} okText="创建交接单" cancelText="取消">
        <Form form={createForm} layout="vertical" onValuesChange={(changed) => {
          if ('specimenId' in changed) {
            setCreateSpecimenId(changed.specimenId)
