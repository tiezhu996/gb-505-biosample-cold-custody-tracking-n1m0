...
import { Alert, Button, Checkbox, Col, DatePicker, Form, Input, Modal, Radio, Row, Segmented, Space, Typography, message } from 'antd'
...
export function ProtocolsPage() {
  ...
  useEffect(() => { void load() }, [])
  const blockedCount = specimens.filter((item) => specimenBlocked(item)).length
  const show = async (specimen: Specimen) => setDrawerSpecimen(await specimenAPI.get(specimen.id))
  const submit = async () => {
    if (!reviewTarget) return
    const values = await form.validateFields()
    if (values.decision === 'approved' && reviewTarget && specimenBlocked(reviewTarget)) {
      message.error('样本处于温度异常处置期间，不能批准放行，请选择暂缓')
      return
    }
    setSaving(true)
    try {
  ...
  const specimenColumns: ColumnsType<Specimen> = [
    ...
    { title: '保管状态', render: (_, row) => <Space direction="vertical" size={2}><CustodyBadge state={row.state} /><SpecimenHoldTag specimen={row} /></Space> },
    { title: '已复核次数', render: (_, row) => row.protocolReviews?.length || 0 },
    { title: '操作', fixed: 'right', render: (_, row) => <Space><Button size="small" icon={<EyeOutlined />} onClick={() => void show(row)}>样本详情</Button>{can('protocol:review') && (specimenBlocked(row)
      ? <Button size="small" type="primary" disabled icon={<FileProtectOutlined />}>异常暂缓放行</Button>
      : <Button size="small" type="primary" icon={<FileProtectOutlined />} onClick={() => { setReviewTarget(row); form.setFieldsValue({ decision: row.state === 'stored' ? 'approved' : 'hold', consentVerified: false, scopeVerified: false }) }}>复核</Button>)}</Space> },
  ]
  ...
  return (
    <div className="page-stack">
      ...
      {mode === 'queue' && blockedCount > 0 && <Alert type="error" showIcon style={{ marginTop: 12 }} message={`有 ${blockedCount} 支样本处于未结案温度异常期间，协议批准放行已暂缓；仍可登记“暂缓/拒绝”复核说明。`} />}
