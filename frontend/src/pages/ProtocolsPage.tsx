...
export function ProtocolsPage() {
  ...
  const specimenSuspended = (specimen: Specimen) => Boolean(specimen.incidentItems?.some((item) => item.incident?.state === 'open'))
  const submit = async () => {
  ...
  const specimenColumns: ColumnsType<Specimen> = [
    ...
    { title: '保管状态', dataIndex: 'state', render: (value, row) => <span><CustodyBadge state={value} />{specimenSuspended(row) && <Tag icon={<WarningOutlined />} color="error">异常暂缓放行</Tag>}</span> },
    { title: '已复核次数', render: (_, row) => row.protocolReviews?.length || 0 },
    { title: '操作', fixed: 'right', render: (_, row) => <Space><Button size="small" icon={<EyeOutlined />} onClick={() => void show(row)}>样本详情</Button>{can('protocol:review') && <Button size="small" type="primary" icon={<FileProtectOutlined />} onClick={() => { setReviewTarget(row); form.setFieldsValue({ decision: specimenSuspended(row) ? 'hold' : (row.state === 'stored' ? 'approved' : 'hold'), consentVerified: false, scopeVerified: false }) }}>复核</Button>}</Space> },
  ]
  ...
  return (
    <div className="page-stack">
      ...
      <Modal width={680} title={`协议复核 · ${reviewTarget?.accessionNo || ''}`} open={Boolean(reviewTarget)} confirmLoading={saving} onOk={() => void submit()} onCancel={() => setReviewTarget(null)} okText="提交复核" cancelText="取消">
        {reviewTarget && specimenSuspended(reviewTarget) && <Alert style={{ marginBottom: 12 }} type="error" showIcon icon={<WarningOutlined />}
          message="样本处于温度异常处置期间，协议放行已暂缓"
          description="处置单结案前不能选择“通过”，可先登记“暂缓”并说明原因。" />}
        <Form form={form} layout="vertical"><Form.Item label="复核决定" name="decision" rules={[{ required: true }]}><Radio.Group optionType="button" buttonStyle="solid" options={[{ value: 'approved', label: '通过', disabled: Boolean(reviewTarget && specimenSuspended(reviewTarget)) }, { value: 'hold', label: '暂缓' }, { value: 'rejected', label: '拒绝' }]} /></Form.Item><Row gutter={16}><Col span={12}><Form.Item name="consentVerified" valuePropName="checked"><Checkbox>已核验有效知情同意</Checkbox></Form.Item></Col><Col span={12}><Form.Item name="scopeVerified" valuePropName="checked"><Checkbox>样本用途符合协议范围</Checkbox></Form.Item></Col></Row><Form.Item name="retentionUntil" label="样本保留期限"><DatePicker showTime style={{ width: '100%' }} /></Form.Item><Form.Item name="documentObjectKey" label="MinIO 协议文件对象键"><Input placeholder="protocols/PR-2026/consent-v2.pdf" /></Form.Item><Form.Item name="notes" label="复核说明" rules={[{ required: true, min: 5 }]}><Input.TextArea rows={4} maxLength={1000} showCount /></Form.Item></Form>
      </Modal>
